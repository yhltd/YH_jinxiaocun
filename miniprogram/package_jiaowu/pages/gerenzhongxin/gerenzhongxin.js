...
Page({
  ...
  queryUserPermissions: function() {
    ...
    wx.getStorage({
      ...
      success: function(gongsiRes) {
        console.log('获取公司名称成功:', gongsiRes.data);
        
        wx.getStorage({
          key: 'system',
          success: function(systemRes) {
            console.log('获取系统名称成功:', systemRes.data);
            ...
            console.log('用户密码:', userInfo ? userInfo.Password : '未获取到');
            console.log('用户名:', userInfo ? userInfo.UserName : '未获取到');
            ...
            if (userInfo && userInfo.UserName && userInfo.Password) {
              that.queryUserAvatarInChunks(companyName, userInfo.UserName, userInfo.Password);
            } else {
          ...
          fail: systemErr => {
            console.error("获取系统名称失败:", systemErr);
            that.setDefaultAvatar();
          }
        });
      },
      fail: gongsiErr => {
        console.error("获取公司名称失败:", gongsiErr);
        that.setDefaultAvatar();
      }
    });
  },
  ...
  queryUserAvatarInChunks: function(companyName, userName, password) {
    ...
    wx.cloud.callFunction({
      name: 'sql_jiaowu',
      data: {
        sql: "SELECT LENGTH(touxiang) as data_length FROM teacher WHERE Company = '" + companyName + "' AND `Password` = '" + password + "' AND `UserName` ='" + userName + "'"
      },
      success: res => {
  ...
  getAvatarByChunks: function(companyName, userName, password, totalLength) {
    ...
    function getNextChunk() {
      ...
      wx.cloud.callFunction({
        name: 'sql_jiaowu',
        data: {
          sql: `SELECT SUBSTRING(touxiang, ${start + 1}, ${chunkSize}) as chunk FROM teacher WHERE Company = '${companyName}' AND \`Password\` = '${password}' AND \`UserName\` ='${userName}'`
        },
        success: res => {
          if (res.result && res.result.length > 0) {
  ...
  queryUserAvatarDirectly: function(companyName, userName, password) {
    ...
    wx.cloud.callFunction({
      name: 'sql_jiaowu',
      data: {
        sql: "SELECT touxiang FROM teacher WHERE Company = '" + companyName + "' AND `Password` = '" + password + "' AND `UserName` ='" + userName + "'"
      },
      success: res => {
        wx.hideLoading();
  ...
  wx.getFileSystemManager().readFile({
    ...
    success(res) {
      ...
      that.checkImageSize(cleanedBase64, function(processedBase64) {
        // 上传到MySQL数据库
        that.uploadAvatarToMySQL(processedBase64);
      });
    },
    fail(err) {
...
uploadAvatarToMySQL: function(base64Image) {
  ...
  // MySQL UPDATE语句 - 适配teacher表
  const sql = `UPDATE teacher SET touxiang = '${base64Image}' WHERE Company = '${companyName}' AND \`Password\` = '${password}' AND \`UserName\` = '${userName}'`;
  
  console.log('执行MySQL SQL:', sql);
  
  wx.cloud.callFunction({
    name: 'sql_jiaowu',  // 使用相同的云函数
    data: {
      sql: sql
    },
    success: res => {
