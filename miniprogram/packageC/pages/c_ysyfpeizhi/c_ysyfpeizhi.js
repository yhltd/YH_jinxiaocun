...
Page({
  ...
  data: {
    ...
    titil : [
      ...
      {text:"应收/应付",width:"300rpx",type:"text",columnName:"ysyf"},
      {text:"项目名称",width:"300rpx",type:"text",columnName:"xiangmumingcheng"},
      {text:"类别",width:"300rpx",type:"text",columnName:"ysyfkemu"},
      {text:"金额",width:"300rpx",type:"number",columnName:"jine"},
    ],
  ...
  init : function(){
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_cw',
      data: {
        query: "SELECT id,ROW_ID, ysyf, xiangmumingcheng, ysyfkemu, jine FROM ( SELECT *, ROW_NUMBER() OVER(ORDER BY id) as ROW_ID FROM ysyfpeizhi WHERE company = '" + _this.data.userInfo.company + "' ) as a WHERE a.ROW_ID > " + ((pageNum-1)*countPage) + " AND a.ROW_ID < " + (pageNum*countPage+1)
      },
      success: res => {
  ...
  getPageCount : function(){
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_cw',
      data: {
        query: "select count(*) as pageCount from ysyfpeizhi where company = '"+_this.data.userInfo.company+"'"
      },
      success: res => {
  ...
  save: function(e){
    ...
    wx.cloud.callFunction({
      name : "sqlServer_cw",
      data : {
        query: "update ysyfpeizhi set ["+column+"] = '"+new_value_input+"' where id = '"+id+"'"
      },
      success : res=>{
        wx.showToast({
          title: "修改成功",
    ...
    wx.showModal({
      ...
      success : res => {
        if (res.confirm) {
          ...
          var sql = "delete from ysyfpeizhi where id in ("
      ...
      wx.showModal({
        ...
        success : res => {
          if(res.confirm){
            var userInfo = _this.data.userInfo
            wx.navigateTo({
              url: '../../pages/c_ysyfpeizhi_insert/c_ysyfpeizhi_insert?userInfo='+JSON.stringify(userInfo),
            })
          }else if(res.cancel){
            return;
          }
  ...
  use_book:function(){
    ...
    wx.showModal({
      title: '使用说明',
      content: '1.点击更多操作后在弹出的窗口中点击删除配置按钮，选中想要删除的数据后点击右下角删除按钮即可删除。\n2.点击更多操作后点击新增配置按钮，在弹出的页面中录入信息点击确定按钮即可添加，录入时多个类别和金额时用英文逗号隔开每一项，类别与金额按照顺序一一对应。\n3.点击页面已有数据的对应列，可弹出修改窗口，录入数据点击确定按钮后即可修改对应位置，多个类别和金额修改时与录入同理。',
  ...
  select:function(e){
    ...
    var sql = "select id,ysyf,xiangmumingcheng,ysyfkemu,jine from (select *,row_number() over(order by id) as ROW_ID from ysyfpeizhi where company = '"+_this.data.userInfo.company+"') as a where  a.ROW_ID > "+(pageNum-1)*countPage+" and a.ROW_ID < "+(pageNum*countPage+1) + " and xiangmumingcheng like '%" + xiangmumingcheng + "%';"
