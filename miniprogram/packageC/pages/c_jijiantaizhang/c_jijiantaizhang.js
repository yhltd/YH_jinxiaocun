...
Page({
  ...
  uploadFile: function() {
    ...
    // 获取公司名
    const companyName = that.data.userInfo?.company || '';
  ...
  startUpload: function(companyName) {
    ...
    // 构建动态路径：/caiwu/ + 公司名 + /
    const dynamicPath = "/caiwu/" + companyName + "/";
    ...
    function uploadNextFile(index) {
      ...
      if (userFileName && userFileName.trim() !== '') {
        ...
        finalFileName = totalFiles === 1 
          ? `台账文件_${safeRecordName}.${fileExtension}` 
          : `台账文件_${safeRecordName}_${index + 1}.${fileExtension}`;
      }
        ...
        success: function(uploadRes) {
          ...
          try {
            ...
            if (resData.code === 200 || resData.success) {
              var fileUrl = "http://yhocn.cn:9088/caiwu/" + companyName + "/" + finalFileName;
              uploadedFiles.push({
                name: finalFileName,
                url: fileUrl,
  ...
  deleteFile: function(e) {
    ...
    var recordId = e.currentTarget.dataset.id;
    var companyName = that.data.userInfo?.company || '';
