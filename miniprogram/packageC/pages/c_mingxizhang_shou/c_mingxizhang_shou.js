Page({
  ...
  get_list:function(list){
    ...
    if (list2.length == 0){
      ...
      heji1 = heji1 + (list2[0].receivable || 0) * 1
      heji2 = heji2 + (list2[0].receipts || 0) * 1
      heji3 = heji3 + (list2[0].weishou || 0) * 1
    }
    ...
    for(var i=0; i<list1.length; i++){
      ...
      heji1 = heji1 + (list1[i].receivable || 0) * 1
      heji2 = heji2 + (list1[i].receipts || 0) * 1
      heji3 = heji3 + (list1[i].weishou || 0) * 1
    }
  ...
  select:function(e){
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_cw',
      data: {
        query: `select a.id,a.company,convert(varchar(20),a.insert_date,120) as insert_date,a.project,a.kehu,a.receivable,a.receipts,a.cope,a.payment,a.receivable-a.receipts as weishou,a.accounting,a.zhaiyao from (select row_number() over(order by id) as rownum,* from SimpleData where company = '${userInfo.company}' and kehu = '${form.kehu}' and (receivable-receipts)<>0 and insert_date >= '${start_date}' and insert_date <= '${stop_date}') as a;
        select 0 as id,'' as company,null as insert_date,'' as project,b.kehu,b.receivable,b.receipts,b.weishou,'' as accounting,'' as zhaiyao from (select kehu,sum(receivable) as receivable,sum(receipts) as receipts,sum(receivable)-sum(receipts) as weishou from SimpleData where company = '${userInfo.company}' and kehu = '${form.kehu}' and insert_date < '${start_date}' group by kehu) as b;`
      },
      success: res => {
        var list = res.result.recordsets
