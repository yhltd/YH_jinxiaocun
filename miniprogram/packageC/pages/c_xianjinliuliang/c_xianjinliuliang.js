...
Page({
  ...
  select:function(e){
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_cw',
      data: {
        query: "select expenditure,isnull((select sum(s.money) from VoucherSummary as s where company = 'admin' and year(voucherDate) = year('"+ start_date +"'+'-01') and month(voucherDate) >= month('" + start_date + "'+'-01') and month(voucherDate) <= month('" + stop_date + "'+'-01') and s.expenditure = v.expenditure),0) as money_month,isnull((select sum(s.money) from VoucherSummary as s where company = '"+userInfo.company+"' and year(voucherDate) = year('" + start_date + "'+'-01') and s.expenditure = v.expenditure),0) as money_year from VoucherSummary as v where company = '"+userInfo.company+"' GROUP BY expenditure"
      },
      success: res => {
        var list = res.result.recordset
        var sum_month = 0;
