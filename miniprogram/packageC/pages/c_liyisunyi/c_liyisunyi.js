...
Page({
  ...
  select:function(e){
    ...
    var sql = "select * from (select name,y.sum_month,y.sum_year,a.direction,row_number() over(order by name) as ROW_ID from Accounting as a,(SELECT code,isnull((SELECT sum(money) FROM VoucherSummary WHERE voucherDate >= CONVERT(date,'"+ start_date +"-01') and voucherDate <= CONVERT(date,'"+ stop_date +"-01') AND code = y.code),0) AS sum_month,isnull((SELECT sum(money) FROM VoucherSummary WHERE YEAR(voucherDate) = YEAR(CONVERT(date,'"+ start_date +"-01')) AND code = y.code),0) AS sum_year FROM VoucherSummary AS y WHERE company = '"+userInfo.company+"' and YEAR(voucherDate) = YEAR(CONVERT(date,'"+ start_date +"-01')) GROUP BY y.code) as y where a.code = y.code and a.company = '"+userInfo.company+"' and a.direction =1) as t where t.ROW_ID > "+(pageNum-1)*countPage+" and t.ROW_ID < "+(pageNum*countPage+1)
    console.log(sql)
