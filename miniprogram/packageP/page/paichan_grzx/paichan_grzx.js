...
Page({
      ...
      success: function(gongsiRes) {
        wx.getStorage({
          ...
          success: function(systemRes) {
            ...
            // 使用分段获取头像数据的方法
            if (userInfo && userInfo.user_code && userInfo.password) {
              that.queryUserAvatarInChunks(companyName, userInfo.user_code, userInfo.password);
            } else {
              console.error('用户信息不完整，无法获取头像');
  ...
  // 分段获取大base64数据的主方法
  queryUserAvatarInChunks: function(companyName, userCode, password) {
    var that = this;
    
    console.log('=== 分段获取头像数据 ===');
    console.log('查询参数:', { companyName, userCode, password });
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_PC',
      data: {
        query: "SELECT LEN(touxiang) as data_length FROM user_info WHERE company = '" + companyName + "' AND user_code = '" + userCode + "' AND password ='" + password + "'"
      },
      success: res => {
        ...
        if (res.result && res.result.recordset && res.result.recordset.length > 0) {
          ...
          if (dataLength > 0) {
            if (dataLength > 500000) { // 如果超过500KB，分段获取
              ...
              that.getAvatarByChunks(companyName, userCode, password, dataLength);
            } else {
              // 直接获取完整数据
              console.log('头像数据较小，直接获取');
              that.queryUserAvatarDirectly(companyName, userCode, password);
            }
          } else {
  ...
  // 分段获取头像数据
  getAvatarByChunks: function(companyName, userCode, password, totalLength) {
    var that = this;
    ...
    function getNextChunk() {
      ...
      wx.cloud.callFunction({
        name: 'sqlServer_PC',
        data: {
          query: `SELECT SUBSTRING(touxiang, ${start + 1}, ${chunkSize}) as chunk FROM user_info WHERE company = '${companyName}' AND user_code = '${userCode}' AND password ='${password}'`
        },
        success: res => {
          if (res.result && res.result.recordset && res.result.recordset.length > 0) {
  ...
  // 直接获取完整头像数据（用于小数据）
  queryUserAvatarDirectly: function(companyName, userCode, password) {
    var that = this;
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_PC',
      data: {
        query: "SELECT touxiang FROM user_info WHERE company = '" + companyName + "' AND user_code = '" + userCode + "' AND password ='" + password + "'"
      },
      success: res => {
        wx.hideLoading();
  ...
  uploadAvatarToDatabase: function(base64Image) {
    ...
    if (!companyName || !userInfo || !userInfo.user_code || !userInfo.password) {
    ...
    // 构建更新SQL - 适配user_info表结构
    const sql = `UPDATE user_info SET touxiang = '${base64Image}' WHERE company = '${companyName}' AND user_code = '${userInfo.user_code}' AND password = '${userInfo.password}'`;
