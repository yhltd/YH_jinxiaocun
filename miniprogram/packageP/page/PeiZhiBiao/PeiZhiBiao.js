...
Page({
  ...
  queryUserPermissions: function() {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "SELECT tptop1,tptop2,tptop3,tptop4,tptop5,tptop6,topgao,xuankuan,xuangao,textbox,beizhu1  FROM yh_notice.dbo.product_pushnews WHERE gsname='济南顺昌制造有限公司' AND  xtname='云合排产管理系统' AND ((qidate IS NULL OR GETUTCDATE() >= CONVERT(DATETIME, LEFT(qidate, 10), 120)) AND (zhidate IS NULL OR GETUTCDATE() <= CONVERT(DATETIME, LEFT(zhidate, 10), 120)))"
      },
      success: res => {
