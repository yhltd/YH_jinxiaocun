// packageA/pages/gerenzhongxin/gerenzhongxin.js
Page({
  ...
  onLoad(options) {
    ...
    console.log("人事数据", userInfo)
    this.queryUserPermissions()
    // 设置用户信息
    _this.setData({
      userInfo: userInfo,
      // 初始化欢迎语
      welcomeText: "欢迎使用云合人事管理系统"
    })
  ...
  generateWelcomeText: function(userInfo) {
    ...
    // 从userInfo中获取公司名称（L字段）
    if (userInfo && userInfo.L && userInfo.L.trim() !== "") {
      var companyName = userInfo.L.trim()
      ...
      // 拼接欢迎语
      var welcomeText = "欢迎使用" + firstFourChars + "人事管理系统"
      ...
      success: function(gongsiRes) {
        wx.getStorage({
          ...
          success: function(systemRes) {
            ...
            // 使用分段获取头像数据的方法
            if (userInfo && userInfo.J && userInfo.I) {
              that.queryUserAvatarInChunks(companyName, userInfo.J, userInfo.I);
            } else {
              console.error('用户信息不完整，缺少 J 或 I 字段');
  ...
  // 分段获取大base64数据的主方法
  queryUserAvatarInChunks: function(companyName, userJ, userI) {
    var that = this;
    
    console.log('=== 分段获取头像数据 ===');
    console.log('查询参数:', { companyName, userJ, userI });
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "SELECT LEN(touxiang) as data_length FROM gongzi_renyuan WHERE L = '" + companyName + "' AND J = '" + userJ + "' AND I ='" + userI + "'"
      },
      success: res => {
        ...
        if (res.result && res.result.recordset && res.result.recordset.length > 0) {
          ...
          if (dataLength > 0) {
            if (dataLength > 500000) { // 如果超过500KB，分段获取
              ...
              that.getAvatarByChunks(companyName, userJ, userI, dataLength);
            } else {
              // 直接获取完整数据
              console.log('头像数据较小，直接获取');
              that.queryUserAvatarDirectly(companyName, userJ, userI);
            }
          } else {
  ...
  // 分段获取头像数据
  getAvatarByChunks: function(companyName, userJ, userI, totalLength) {
    var that = this;
    ...
    function getNextChunk() {
      ...
      wx.cloud.callFunction({
        name: 'sqlServer_117',
        data: {
          query: `SELECT SUBSTRING(touxiang, ${start + 1}, ${chunkSize}) as chunk FROM gongzi_renyuan WHERE L = '${companyName}' AND J = '${userJ}' AND I ='${userI}'`
        },
        success: res => {
          if (res.result && res.result.recordset && res.result.recordset.length > 0) {
  ...
  // 直接获取完整头像数据（用于小数据）
  queryUserAvatarDirectly: function(companyName, userJ, userI) {
    var that = this;
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "SELECT touxiang FROM gongzi_renyuan WHERE L = '" + companyName + "' AND J = '" + userJ + "' AND I ='" + userI + "'"
      },
      success: res => {
        wx.hideLoading();
  ...
  uploadAvatarToDatabase: function(base64Image) {
    ...
    if (!companyName || !userInfo || !userInfo.J || !userInfo.I) {
    ...
    // 构建更新SQL - 适配gongzi_renyuan表结构
    const sql = `UPDATE gongzi_renyuan SET touxiang = '${base64Image}' WHERE L = '${companyName}' AND J = '${userInfo.J}' AND I = '${userInfo.I}'`;
