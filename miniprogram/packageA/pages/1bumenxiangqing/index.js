Page({
  ...
  onLoad: function (options) {
    ...
    var result = JSON.parse(options.access)
    _this.setData({
      department,
      companyName,
      result
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "select top 100 * from gongzi_gongzimingxi where C = '" + department + "' and BD = '"+companyName+"'"
      },
      success: res => {
        console.log("进入成功")
  ...
  onReady: function () {
    var that = this
    //这段代码突然看见谁都会感觉迷糊，这是用来查找下拉框的数组的 option01就是部门下拉框， option02就是岗位下拉框
    ...
    if (index == 0) {
      ...
      if(input == ''){
        ...
        wx.cloud.callFunction({
          name: "sqlServer_117",
          data: {
            query: "select top 100 * from gongzi_gongzimingxi where B LIKE '%" + input + "%'and C='" + that.data.title_bumen + "'and D='" + that.data.title_gangwei + "' and BD = '"+that.data.companyName+"'"
          },
          success: res => {
            console.log("姓名查询成功！", res.result)
            that.setData({
        ...
        wx.cloud.callFunction({
          name: "sqlServer_117",
          data: {
            query: "select top 100 * from gongzi_gongzimingxi where B LIKE '%" + input + "%' and D='" + that.data.title_gangwei + "' and BD = '"+that.data.companyName+"'"
          },
          success: res => {
            console.log("姓名查询成功！", res.result)
            that.setData({
        ...
        wx.cloud.callFunction({
          name: "sqlServer_117",
          data: {
            query: "select top 100 * from gongzi_gongzimingxi where B LIKE '%" + input + "%' and C='" + that.data.title_bumen + "' and BD = '"+that.data.companyName+"'"
          },
          success: res => {
            console.log("姓名查询成功！", res.result)
            that.setData({
  ...
  lastpage: function () {
    ...
    if (that.data.page == 1) {
      ...
      wx.cloud.callFunction({
        name: 'sqlServer_117',
        data: {
          query: "select top 100 * from(select row_number() over(order by cast(id as int) asc) as rownumber, * from gongzi_gongzimingxi where C = '"+that.data.department+"') temp_row where rownumber > (( '1' - 1) * 100) and temp_row.BD = '"+that.data.companyName+"'"
        },
        success: res => {
  ...
  */
  ...
  tianjia: function () {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "insert into gongzi_gongzimingxi (B,C,BD) values('请输入','"+that.data.department+"','"+that.data.companyName+"')"
      },
      success: res => {
        console.log("插入成功")
        that.setData({
