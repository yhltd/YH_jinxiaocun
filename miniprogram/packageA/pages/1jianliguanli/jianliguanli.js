...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "select top 100 id, touliren, xueli, mubiaogangwei, tijiaoshijian, wenjian, beizhu, zhuangtai from gongzi_jianliguanli where gongsi = '"+_this.data.companyName+"' and (zhuangtai is null or zhuangtai = '' or zhuangtai = '待处理' or zhuangtai = '驳回') order by id desc"
      },
      success: res => {
        console.log("进入成功")
  ...
  onReady: function () {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "select count(id) as maxpagenumber from gongzi_jianliguanli where gongsi = '"+that.data.companyName+"' and (zhuangtai is null or zhuangtai = '' or zhuangtai = '待处理' or zhuangtai = '驳回')"
      },
      success: res => {
        that.setData({
  ...
  lastpage: function () {
    ...
    if (that.data.page == 1) {
      ...
      wx.cloud.callFunction({
        name: 'sqlServer_117',
        data: {
          query: "select top 100 * from(select row_number() over(order by cast(id as int) asc) as rownumber, * from gongzi_jianliguanli) temp_row where rownumber > (( '" + that.data.page + "' - 1) * 100) and gongsi = '"+that.data.companyName+"' and (zhuangtai is null or zhuangtai = '' or zhuangtai = '待处理' or zhuangtai = '驳回')"
        },
        success: res => {
