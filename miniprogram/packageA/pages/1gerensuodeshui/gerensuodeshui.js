Page({
  ...
  onLoad: function () {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "select top 100 AU,sum((cast(ATA as money)*cast(AU as money))) AS COUNT1,COUNT(id)as num, sum((cast(AW as money)*cast(AU as money))) AS COUNT2 from gongzi_gongzimingxi where AU is not null group by AU"
      },
      success: res => {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "select gerensuodeshui from title where gerensuodeshui is not null"
      },
      success: res => {
        this.setData({
  ...
  onReady: function () {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "select count(1) as maxpagenumber from (select top 100 AU,sum((cast(ATA as money)*cast(AU as money))) AS COUNT1,COUNT(id)as num, sum((cast(AW as money)*cast(AU as money))) AS COUNT2 from gongzi_gongzimingxi where AU is not null group by AU)t1"
      },
      success: res => {
        that.setData({
          maxpagenumber: Math.ceil(res.result.recordset[0].maxpagenumber / 100),
          listLength: res.result.recordset[0].maxpagenumber
        })
  ...
  analysis: function (e) {
    ...
    for (var i = 0; i < that.data.listLength; i++) {
      that.data.analysis[0].jishui += that.data.list[i].COUNT1
      that.data.analysis[0].renshu +=that.data.list[i].num
      that.data.analysis[0].gerensuodeshui += that.data.list[i].COUNT2
    }
    that.setData({
  ...
  lastpage: function () {
    ...
    if (that.data.page == 1) {
      ...
      wx.cloud.callFunction({
        name: 'sqlServer_117',
        data: {
          query: "select top 100 * from(select row_number() over(order by cast(id as int) asc) as rownumber, AU,sum((cast(ATA as money)*cast(AU as money))) AS COUNT1,COUNT(id)as num, sum((cast(AW as money)*cast(AU as money))) AS COUNT2 from gongzi_gongzimingxi group by AU) temp_row where rownumber > (( '" + that.data.page + "' - 1) * 100);"
        },
        success: res => {
