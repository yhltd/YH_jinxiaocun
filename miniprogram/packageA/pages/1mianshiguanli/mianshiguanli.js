...
Page({
  ...
  onReady: function () {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "select count(id) as maxpagenumber from gongzi_jianliguanli where gongsi = '"+that.data.companyName+"' and zhuangtai = '通过'"
      },
      success: res => {
        that.setData({
  ...
  kuaisutianjia: function () {
    ...
    wx.cloud.callFunction({
      name: 'sqlServer_117',
      data: {
        query: "insert into gongzi_jianliguanli (gongsi, tijiaoshijian, zhuangtai) values('"+that.data.companyName+"', CONVERT(varchar(10), GETDATE(), 120), '通过')"
      },
      success: res => {
        console.log("插入成功!!!!!!")
        that.baochi()
  ...
  lastpage: function () {
    ...
    if (that.data.page == 1) {
      ...
      wx.cloud.callFunction({
        name: 'sqlServer_117',
        data: {
          query: "select top 100 * from(select row_number() over(order by cast(id as int) asc) as rownumber, * from gongzi_jianliguanli) temp_row where rownumber > (( '" + that.data.page + "' - 1) * 100) and gongsi = '"+that.data.companyName+"' and zhuangtai = '通过'"
        },
        success: res => {
