...
Page({
      ...
      success: function(gongsiRes) {
        ...
        wx.getStorage({
          ...
          success: function(systemRes) {
            ...
            // 检查必要的参数是否存在
            if (!userInfo || !userInfo.password || !userInfo.uname) {
              console.error('用户信息不完整，缺少密码或账号字段');
            ...
            // 开始分段获取头像数据
            that.queryUserAvatarInChunks(companyName, userInfo.uname, userInfo.password);
          },
          fail: systemErr => {
  ...
  queryUserAvatarInChunks: function(companyName, userName, password) {
    ...
    wx.cloud.callFunction({
      ...
      data: {
        sql: "SELECT LENGTH(touxiang) as data_length FROM users WHERE company = '" + companyName + "' AND password = '" + password + "' AND account ='" + userName + "'"
      },
      success: res => {
  ...
  getAvatarByChunks: function(companyName, userName, password, totalLength) {
    ...
    function getNextChunk() {
      ...
      wx.cloud.callFunction({
        name: 'sqlserver_xinyongka',
        data: {
          sql: `SELECT SUBSTRING(touxiang, ${start + 1}, ${chunkSize}) as chunk FROM users WHERE company = '${companyName}' AND password = '${password}' AND account ='${userName}'`
        },
        success: res => {
          if (res.result && res.result.length > 0) {
  ...
  queryUserAvatarDirectly: function(companyName, userName, password) {
    ...
    wx.cloud.callFunction({
      name: 'sqlserver_xinyongka',
      data: {
        sql: "SELECT touxiang FROM users WHERE company = '" + companyName + "' AND password = '" + password + "' AND account ='" + userName + "'"
      },
      success: res => {
        wx.hideLoading();
...
convertImageToBase64: function(tempFilePath) {
  ...
  wx.getFileSystemManager().readFile({
    ...
    success(res) {
      ...
      // 检查图片大小后再上传
      that.checkImageSize(cleanedBase64, function(checkedBase64) {
        that.uploadAvatarToMySQL(checkedBase64);
      });
    },
    fail(err) {
...
uploadAvatarToMySQL: function(base64Image) {
  ...
  // 从页面数据中获取用户信息，而不是从app.globalData
  const companyName = this.data.gongsi;
  const userInfo = this.data.userInfo;
  
  if (!companyName || !userInfo || !userInfo.uname || !userInfo.password) {
  ...
  // 转义base64数据中的单引号，防止SQL注入
  const escapedBase64 = base64Image.replace(/'/g, "''");
  
  // MySQL UPDATE语句 - 使用正确的表名和字段名
  const sql = `UPDATE users SET touxiang = '${escapedBase64}' WHERE company = '${companyName}' AND password = '${password}' AND account = '${userName}'`;
  ...
  wx.cloud.callFunction({
    ...
    fail: err => {
      ...
      } else if (err.errMsg && err.errMsg.includes('timed out')) {
        wx.showToast({
          title: '上传超时，请重试',
          icon: 'none',
          duration: 2000
