...
SELECT * FROM (
  SELECT 
    ROW_NUMBER() OVER (ORDER BY jxc.cpid) AS row_num,
  ...
  FROM (
    SELECT 
      ...
      ISNULL(CAST(link_rk.cpsl AS DECIMAL(18,2)),0) as qcsl,
      ISNULL(CAST(link_rk.cpje AS DECIMAL(18,2)),0) as qcje,
      ISNULL(CAST(link_rk.rksl AS DECIMAL(18,2)),0) as rksl,
      ISNULL(CAST(link_rk.rkje AS DECIMAL(18,2)),0) as rkje,
      ISNULL(CAST(ck.cksl AS DECIMAL(18,2)),0) as cksl,
      ISNULL(CAST(ck.ckje AS DECIMAL(18,2)),0) as ckje,
      ISNULL(CAST(link_rk.cpsl AS DECIMAL(18,2)),0) + ISNULL(CAST(link_rk.rksl AS DECIMAL(18,2)),0) - ISNULL(CAST(ck.cksl AS DECIMAL(18,2)),0) as jcsl,
      ISNULL(CAST(link_rk.cpje AS DECIMAL(18,2)),0) + ISNULL(CAST(link_rk.rkje AS DECIMAL(18,2)),0) - ISNULL(CAST(ck.ckje AS DECIMAL(18,2)),0) as jcje
    FROM (
      SELECT 
        link_qc.cpid, link_qc.cpname, link_qc.cplb, link_qc.cpsl, link_qc.cpje,
        rk.rksl, rk.rkje
      FROM (
        SELECT 
          cp.cpid, cp.cpname, cp.cplb, qc.cpsl, qc.cpje
        FROM (
          SELECT cpid, cpname, cplb FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_qichushu_mssql WHERE gs_name = '${gongsi}'
          UNION 
          SELECT sp_dm, cpname, cplb FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql WHERE gs_name = '${gongsi}'
        ) AS cp 
        LEFT JOIN (
          SELECT cpid, cplb, cpname, SUM(CAST(cpsl AS DECIMAL(18,2))) AS cpsl, SUM(CAST(cpsj AS DECIMAL(18,2)) * CAST(cpsl AS DECIMAL(18,2))) AS cpje 
          FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_qichushu_mssql WHERE gs_name = '${gongsi}'
          GROUP BY cpid, cpname, cplb
        ) AS qc ON cp.cpid = qc.cpid AND cp.cpname = qc.cpname AND cp.cplb = qc.cplb
      ) AS link_qc 
      LEFT JOIN (
        SELECT sp_dm, cpname, cplb, SUM(CAST(cpsl AS DECIMAL(18,2))) AS rksl, SUM(CAST(cpsl AS DECIMAL(18,2)) * CAST(cpsj AS DECIMAL(18,2))) AS rkje
        FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql 
        WHERE mxtype IN ('入库', '调拨入库', '盘盈入库') AND gs_name = '${gongsi}' 
        AND shijian BETWEEN '${start_date}' AND '${stop_date}'
        GROUP BY sp_dm, cpname, cplb
      ) AS rk ON rk.sp_dm = link_qc.cpid AND rk.cpname = link_qc.cpname AND rk.cplb = link_qc.cplb
    ) AS link_rk 
    LEFT JOIN (
      SELECT sp_dm, cpname, cplb, SUM(CAST(cpsl AS DECIMAL(18,2))) AS cksl, SUM(CAST(cpsl AS DECIMAL(18,2)) * CAST(cpsj AS DECIMAL(18,2))) AS ckje
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql 
      WHERE mxtype IN ('出库', '调拨出库', '盘亏出库') AND gs_name = '${gongsi}'
      AND shijian BETWEEN '${start_date}' AND '${stop_date}'
      GROUP BY sp_dm, cpname, cplb
    ) AS ck ON ck.sp_dm = link_rk.cpid AND ck.cpname = link_rk.cpname AND ck.cplb = link_rk.cplb
  ) AS jxc 
  ...
  WHERE CAST(jxc.cksl AS DECIMAL(18,2)) < ${order_number}
) AS t
WHERE t.row_num BETWEEN ${page * 5 + 1} AND ${page * 5 + 5}
ORDER BY t.cpid
`
    ...
    wx.cloud.callFunction({
      ...
      data: { query: step1Sql },
      success(step1Res) {
        console.log('✅ 基础查询成功，数据条数:', step1Res.result.recordset.length)
        if (step1Res.result.recordset.length > 0) {
          console.log('cangku字段示例:', step1Res.result.recordset[0].cangku)
        }
        
        // 第二步：添加业务员关联
        _this.testWithBianyuan(gongsi, start_date, stop_date, order_number, page, step1Res.result.recordset)
      },
      fail(step1Res) {
        console.log('❌ 基础查询失败')
        console.log('错误信息:', JSON.stringify(step1Res, null, 2))
...
`
  ...
  SELECT * FROM (
    ...
    FROM (
      -- 所有商品ID（去重）
      SELECT DISTINCT sp_dm as cpid, cpname, cplb 
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql WITH (NOLOCK)
      WHERE gs_name = '${gongsi}'
      UNION
      SELECT DISTINCT cpid, cpname, cplb 
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_qichushu_mssql WITH (NOLOCK)
      WHERE gs_name = '${gongsi}'
    ) AS jxc
    
    -- 期初库存
    LEFT JOIN (
      SELECT cpid, cpname, cplb,
        SUM(CAST(cpsl AS DECIMAL(18,2))) as qcsl,
        SUM(CAST(cpsl AS DECIMAL(18,2)) * CAST(cpsj AS DECIMAL(18,2))) as qcje
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_qichushu_mssql WITH (NOLOCK)
      WHERE gs_name = '${gongsi}'
      GROUP BY cpid, cpname, cplb
    ) AS qc ON jxc.cpid = qc.cpid AND jxc.cpname = qc.cpname AND jxc.cplb = qc.cplb
    ...
    LEFT JOIN (
      ...
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql WITH (NOLOCK)
      WHERE gs_name = '${gongsi}'
        AND mxtype IN ('入库', '调拨入库', '盘盈入库')
        AND shijian BETWEEN '${start_date}' AND '${stop_date}'
      GROUP BY sp_dm, cpname, cplb
    ) AS rk ON jxc.cpid = rk.sp_dm AND jxc.cpname = rk.cpname AND jxc.cplb = rk.cplb
    ...
    LEFT JOIN (
      ...
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql WITH (NOLOCK)
      WHERE gs_name = '${gongsi}'
        AND mxtype IN ('出库', '调拨出库', '盘亏出库')
        AND shijian BETWEEN '${start_date}' AND '${stop_date}'
      GROUP BY sp_dm, cpname, cplb
    ) AS ck ON jxc.cpid = ck.sp_dm AND jxc.cpname = ck.cpname AND jxc.cplb = ck.cplb
    
    -- 仓库信息
    LEFT JOIN (
      SELECT DISTINCT sp_dm, cangku
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql WITH (NOLOCK)
      WHERE gs_name = '${gongsi}'
    ) AS cangku_info ON jxc.cpid = cangku_info.sp_dm
    ...
    LEFT JOIN (
      SELECT sp_dm, lei_bie, name, bianyuan, mark1
      FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_jichuziliao_mssql WITH (NOLOCK)
      WHERE gs_name = '${gongsi}'
    ) AS bian_yuan ON jxc.cpid = bian_yuan.sp_dm 
      AND jxc.cpname = bian_yuan.name 
      AND jxc.cplb = bian_yuan.lei_bie
    
    WHERE ISNULL(ck.cksl, 0) < ${order_number}
  ) AS t
  WHERE t.row_num BETWEEN ${page * 5 + 1} AND ${page * 5 + 5}
  ORDER BY t.cpid
`
    ...
    wx.cloud.callFunction({
      ...
      data: { query: step2Sql },
      success(step2Res) {
        console.log('返回内容:', step2Res)
        if (step2Res.result.recordset.length > 0) {
          console.log('cangku字段示例:', step2Res.result.recordset[0].cangku)
        }
        
        // 第三步：添加完整计算字段（现在cangku已经在前面获取了）
        _this.testWithFullCalculations(gongsi, start_date, stop_date, order_number, page, step2Res.result.recordset)
      },
      fail(step2Res) {
        console.log('❌ 业务员关联查询失败')
        console.log('错误信息:', JSON.stringify(step2Res, null, 2))
...
calculateYjdjForItem: function(gongsi, item) {
  return new Promise((resolve, reject) => {
    ...
    wx.cloud.callFunction({
      ...
      var sql = `
      select 
        ISNULL(CAST(qc.cpje AS DECIMAL(18,2)), 0) as qichu_je,
        ISNULL(CAST(ck.cpsj AS DECIMAL(18,2)), 0) as last_ck_price
      from (
        select cpid, cplb, cpname, SUM(CAST(cpsj AS DECIMAL(18,2)) * CAST(cpsl AS DECIMAL(18,2))) as cpje 
      ...
      left join (
        select top 1 sp_dm, cpname, cplb, cpsj
        from yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql 
        where mxtype = '出库' 
        and sp_dm = '${item.cpid}' 
        and cpname = '${item.cpname}' 
    ...
    wx.cloud.callFunction({
      ...
      success(res) {
        if (res.result.recordset && res.result.recordset.length > 0) {
          var result = res.result.recordset[0]
          var yjdj = item.jcsl * (result.qichu_je - result.last_ck_price)
          resolve(yjdj || 0)
        } else {
          resolve(0)
...
calculateLdxlForItem: function(gongsi, start_date, stop_date, item) {
  ...
  wx.cloud.callFunction({
    ...
    var sql = `
    select 
      ISNULL(CAST(qc.cpsl AS DECIMAL(18,2)), 0) as qichu_sl,
      ISNULL(CAST(rk.rksl AS DECIMAL(18,2)), 0) as ruku_sl,
      ISNULL(CAST(ck.cksl AS DECIMAL(18,2)), 0) as chuku_sl,
      -- 计算期末库存（stop_date 之前的累计库存）
      (ISNULL(CAST(qc.cpsl AS DECIMAL(18,2)), 0) + ISNULL(CAST(all_rk.rksl AS DECIMAL(18,2)), 0) - ISNULL(CAST(all_ck.cksl AS DECIMAL(18,2)), 0)) as qimo_sl
    from (
      select cpid, cplb, cpname, SUM(CAST(cpsl AS DECIMAL(18,2))) as cpsl 
      from yh_jinxiaocun_excel.dbo.yh_jinxiaocun_qichushu_mssql 
      where gs_name = '${gongsi}' 
      and cpid = '${item.cpid}' 
      and cpname = '${item.cpname}' 
    ...
    left join (
      select sp_dm, cpname, cplb, SUM(CAST(cpsl AS DECIMAL(18,2))) as rksl
      from yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql 
      where mxtype = '入库' 
      and gs_name = '${gongsi}' 
      and sp_dm = '${item.cpid}' 
    ...
    left join (
      select sp_dm, cpname, cplb, SUM(CAST(cpsl AS DECIMAL(18,2))) as cksl
      from yh_jinxiaocun_excel.dbo.yh_jinxiaocun_mingxi_mssql 
      where mxtype = '出库' 
      and gs_name = '${gongsi}' 
      and sp_dm = '${item.cpid}' 
