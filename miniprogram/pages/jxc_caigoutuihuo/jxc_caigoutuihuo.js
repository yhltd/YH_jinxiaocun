...
Page({
  ...
  sel1:function(){
    ...
    if(app.globalData.shujuku==0){
      ...
      wx.cloud.callFunction({
        name: "sqlConnection",
        data: {
          sql: "SELECT yh_jinxiaocun_tuihuomingxi.id as tuihuo_id, yh_jinxiaocun_tuihuomingxi.*, '' as checkbox, date_format(yh_jinxiaocun_tuihuomingxi.shijian, '%Y-%m-%d') as time, date_format(yh_jinxiaocun_tuihuomingxi.shijian, '%Y-%m-%d %H:%i:%s') as time2, yh_jinxiaocun_jichuziliao.mark1 as mark1, yh_jinxiaocun_tuihuomingxi.cangku as cangku FROM yh_jinxiaocun_tuihuomingxi LEFT JOIN yh_jinxiaocun_jichuziliao ON yh_jinxiaocun_tuihuomingxi.cpname =yh_jinxiaocun_jichuziliao.`name` WHERE yh_jinxiaocun_tuihuomingxi.gs_name = '" + gongsi + "' AND shijian >= '" + start_date + "' AND shijian <= '" + stop_date + "' AND orderid LIKE '%" + order_number + "%' AND (mxtype = '采购' ) LIMIT "+page+", 5"
         },
        success(res) {
 ...
 var page = _this.data.page
        ...
        if (res.confirm) {
          ...
          if(app.globalData.shujuku==0){
            ...
            wx.cloud.callFunction({
              name: "sqlConnection",
              data: {
                sql: "DELETE FROM yh_jinxiaocun_tuihuomingxi where id = '" + uid + "'"
              },
              success: res=> {
                wx.showToast({
                  title: '删除成功',
            ...
            wx.cloud.callFunction({
              name: "sqlServer_117",
              data: {
                query: "DELETE FROM yh_jinxiaocun_excel.dbo.yh_jinxiaocun_tuihuomingxi_mssql where id = '" + uid + "'"
              },
              success: res=> {
                wx.showToast({
                  title: '删除成功',
  ...
  upd: function (e) {
    ...
    console.log(id)
    if(id != undefined){
      var uid = _this.data.szzhi[id].id
      console.log(uid)
      wx.navigateTo({
        url: '/pages/caigoutuihuo_update/caigoutuihuo_update?id=' + uid + '&fun=update',
      })
    }else{
