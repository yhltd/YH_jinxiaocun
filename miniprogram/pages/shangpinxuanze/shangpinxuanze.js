        ...
        data: {
          ...
          SELECT 
            ...
            ISNULL((
                ...
                FROM [yh_jinxiaocun_excel].[dbo].[yh_jinxiaocun_mingxi_mssql] m
                WHERE m.cpname = j.name 
                    AND m.gs_name = j.gs_name
            ), 0) as allSL
        FROM [yh_jinxiaocun_excel].[dbo].[yh_jinxiaocun_jichuziliao_mssql] j
        ...
        success(res) {
          ...
          if(wx.getStorageSync('sz') != undefined && wx.getStorageSync('sz') != ""){
            ...
            for(let i=0;i<res.result.recordset.length;i++){
              if(sz[res.result.recordset[i].id] != undefined){
                that.setData({
                  ["all["+i+"].allSL"]: res.result.recordset[i].allSL - parseInt(sz[res.result.recordset[i].id])
                })
              }
  ...
  querenRk: function() {
    ...
    if(_this.data.fun!='qichu' ){
      ...
      for(let i=0;i<szZhi.length;i++){
        for(let j=0;j<dms.length;j++){
          if(szZhi[i].sp_dm==dms[j]){
            ssql +="INSERT INTO yh_jinxiaocun_excel.dbo.yh_jinxiaocun_qichushu_mssql (cpid,cplb,cpname,cpsj,cpsl,zh_name,gs_name,shijian,mark1,cangku) VALUES ('" + szZhi[i].sp_dm + "','" + szZhi[i].lei_bie + "','" + szZhi[i].name + "','" + cpjg[i] + "','" + cpsl[i] + "','" + app.globalData.finduser + "','" + app.globalData.gongsi + "','" + new Date() + "','" + szZhi[i].mark1 + "','" + cangku[i] + "');"
            break;
          }
        }
      }
