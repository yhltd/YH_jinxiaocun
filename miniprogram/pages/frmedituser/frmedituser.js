...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.cloud.callFunction({
      name: "sqlConnection",
      data: {
        sql: "select * from yh_jinxiaocun_user where name = '" + this.data.editid + "' and gongsi = '" + gongsi + "'"
      },
      success(res) {
        console.log("成功", res.result[0])
        that.setData({
          listAll: res.result[0]
        })
      },
      fail(res) {
        console.log("失败", res)
  ...
  searchBox: function(e) {
    ...
    wx.cloud.callFunction({
      name: "sqlConnection",
      data: {
        sql: "update yh_jinxiaocun_user set name = '" + uname + "' ,password = '" + pass + "', AdminIS = '" + that.data.AdminIS + "',Btype = '" + that.data.Btype + "',gongsi='" + gongsi + "', jigoudaima = '" + jigoudaima + "',Createdate = '" + time + "'  where gongsi = '" + gongsi + "' and _id ='" + this.data.editid + "'"
      },
      success(res) {
        console.log("成功")
        wx.showToast({
