...
Page({
  ...
  setCanvas: function (comment_order) {
    ...
    ctx.fillText('日期：' + order.date, 0, 20)
    ctx.setTextAlign('left')
    ctx.fillText('客户：' + order.customer_name, 0, 40)
    ctx.setTextAlign('left')
    ctx.fillText('订单号：' + order.customer_order, 0, 60)
    ctx.setTextAlign('left')
    ctx.fillText('客户需求：' + order.customer_need_text, 0, 80)
    ctx.draw()
  },
  ...
  printTo : function(){
    ...
    printUtil.printlnText(4, 15, height, '日期：' + order.date);
    height = height + 40
    printUtil.printlnText(4, 15, height, '客户：' + order.customer_name);
    height = height + 40
    printUtil.printlnText(4, 15, height, '订单号：' + order.customer_order);
    height = height + 40
    printUtil.printlnText(4, 15, height, '需求：' + order.customer_need_text);
    height = height + 40
